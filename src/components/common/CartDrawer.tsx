...
import { removeItem, updateQuantity } from '@/lib/store/features/cart/cartSlice';
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            {items.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-bold text-[#6B4A2D]">Your cart is empty</h3>
                                        <p className="text-[#6B4A2D]/60 text-sm mt-1 mb-6">Looks like you haven't added anything yet.</p>
                                        <Button
                                            onClick={onClose}
                                        ...
                                        <div className="flex-1 flex flex-col justify-between">
                                            <div>
                                                <div className="flex justify-between items-start gap-2">
                                                    <h3 className="font-bold text-[#6B4A2D] line-clamp-1">{item.product.name}</h3>
                                                    <button
                                                ...
                                                <p className="text-sm text-[#6B4A2D]/60">{item.product.category?.name}</p>
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between mt-3">
                                                <div className="flex items-center gap-3 bg-[#F5F5F0] rounded-lg p-1">
                                                    ...
                                                    <span className="text-sm font-bold text-[#6B4A2D] w-4 text-center">{item.quantity}</span>
                                                    <button
                                                        onClick={() => dispatch(updateQuantity({ productId: item.productId, quantity: item.quantity + 1, variantId: item.variantId }))}
