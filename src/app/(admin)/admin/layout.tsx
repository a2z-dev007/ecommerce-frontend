        ...
        <nav className="flex flex-col gap-6 p-4 h-[calc(100vh-72px)] overflow-y-auto no-scrollbar">
                    ...
                    <div key={item.name} className="space-y-0.5">
                      {hasChildren ? (
                        <div className="space-y-0.5">
                          <Button
                            ...
                            className={cn(
                              ...
                              isExpanded ? 'text-primary' : 'text-slate-600 dark:text-slate-400'
                            )}
                          >
                            <div className="flex items-center">
                              <Icon className={cn('mr-3 h-[18px] w-[18px] transition-colors', isExpanded ? 'text-primary' : 'text-slate-400 dark:text-slate-500 group-hover:text-primary')} />
                                ...
                                return (
                                  <Link key={child.name} href={child.href}>
                                    <div className={cn(
                                      ...
                                      isChildActive ? 'text-primary' : 'text-slate-500 hover:text-primary dark:text-slate-500 dark:hover:text-primary'
                                    )}>
                                      {child.name}
                                    </div>
                                  </Link>
                        ...
                        <Link href={item.href || '#'}>
                          ...
                          >
                            <Icon className={cn('mr-3 h-[18px] w-[18px] transition-colors', isActive ? 'text-primary' : 'text-slate-400 dark:text-slate-500 group-hover:text-primary')} />
                            <span className="text-[14px] font-medium">{item.name}</span>
                            {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-primary shadow-sm shadow-primary/50" />}
                          </Button>
                        </Link>
                      )}
                    </div>
          ...
          <div className="mt-auto pt-6 px-2">
            <div className="bg-primary/5 dark:bg-primary/10 rounded-2xl p-5 border border-primary/10 text-center space-y-3 relative overflow-hidden group">
              ...
              <p className="text-[13px] font-semibold text-slate-900 dark:text-white relative z-10">Hi, how can we help?</p>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 relative z-10">Contact us if you have any assistance.</p>
              <Button size="sm" className="w-full h-9 rounded-xl text-[12px] relative z-10 font-bold">
                Contact
              </Button>
            </div>
        ...
        <header className="flex h-[72px] items-center justify-between bg-white dark:bg-slate-900 px-6 border-b border-slate-100 dark:border-slate-800 sticky top-0 z-30 shadow-sm shadow-slate-100/50 dark:shadow-none">
          ...
          <div className="flex items-center gap-2">
            <div className="hidden sm:flex items-center gap-1 mr-2 border-r border-slate-100 dark:border-slate-800 pr-2">
              <Button variant="ghost" size="icon" className="h-10 w-10 text-slate-500 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800">
                <Globe className="h-[18px] w-[18px]" />
              </Button>
              <Button variant="ghost" size="icon" className="h-10 w-10 text-slate-500 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800">
                <MessageSquare className="h-[18px] w-[18px]" />
              </Button>
              <Button variant="ghost" size="icon" className="h-10 w-10 text-slate-500 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 relative">
